...
Πληκτρολογούμε ή επιλέγουμε τον φάκελο `Documents/c_programs` που
...
Για να μεταγλωττίσουμε και να τρέξουμε το πρόγραμμά μας, Ctrl-Shift-B.
Αν το πρόγραμμά μας έχει συντακτικά λάθη ή επισημάνσεις, τότε στο κάτω
...
αντιγράφει από και προς απομακρυσμένους υπολογιστές. Προκειμένου να αντιγράψουμε
κάποιο αρχείο, ακολουθούμε την σύνταξη:
...
Με αυτό το μήνυμα ζητείται η επιβεβαίωσή μας για την μεταφορά του
...
όπως μετονομασία των αρχείων, δημιουργία καταλόγων, διαγραφή αρχείων και
καταλόγων κ.λπ.

Όταν ολοκληρώσουμε τις εργασίες μας, πατάμε το κουμπί `Disconnect` για
να αποσυνδεθούμε.
...
αφαίρεση ανάλογα με το τι προτιμάει ο χρήστης. Πώς θα σχεδιάζατε την εφαρμογή
σας;
...
κλείσιμο κάποιου αγκίστρου, η χρήση μιας μεταβλητής που δεν έχουμε
...
έναν χαρακτήρα `"`. Όντως, αν το κοιτάξουμε καλά, λείπει το κλείσιμο του `"`
...
προσπαθήστε να καταλάβετε πώς σχετίζονται με αυτό που κάνατε. Ήταν όλα
...
καμία επιπλέον υπόδειξη. Το λάθος είναι ότι δεν έχουμε κλείσει το
άγκιστρο της main, οπότε ίσως να αναμέναμε κάτι σαν “syntax error, bracket
needed”.
