...
# Εργαστήριο #3: Μεταβλητές, Δομές Ελέγχου και Επανάληψης

> **Στόχοι**
>
> Μετά το εργαστήριο αυτό θα μπορείτε:
>
> - Να επιλέγετε τον κατάλληλο τύπο δεδομένων και να τον τυπώνετε σωστά με τη `printf`.
> - Να χρησιμοποιείτε τις δομές επανάληψης `while`, `for` και `do...while`.
> - Να ελέγχετε τη ροή του προγράμματός σας με `if...else`.
> - Να οργανώνετε τον κώδικά σας σε συναρτήσεις.
> - Να εντοπίζετε λογικά λάθη με ιχνηλάτηση μέσω `printf`.
>
> **Προαπαιτούμενα:** [Εργαστήριο #2](https://progintro.github.io/lab-material/labs/lab02/) - μεταγλώττιση και εκτέλεση προγραμμάτων C.
>
> **Αρχεία που θα φτιάξετε:** `seq.c`, `root.c`, `birthdate.c`, `limit.c`
...
Η C μάς δίνει τρεις δομές επανάληψης. Και οι τρεις εκτελούν επαναληπτικά ένα μπλοκ εντολών - διαφέρουν στο **πότε** ελέγχεται η συνθήκη και στο πού γράφονται η αρχικοποίηση και το βήμα. Για καθεμιά δίνουμε παρακάτω, μαζί με τη σύνταξή της, και το **διάγραμμα ροής** της: μια σχηματική αναπαράσταση της σειράς με την οποία εκτελούνται οι εντολές, όπου ο ρόμβος δηλώνει τον έλεγχο μιας συνθήκης και τα βέλη τη ροή της εκτέλεσης.

Η `while` ελέγχει τη συνθήκη `E2` **πριν** από κάθε επανάληψη. Αν η `E2` είναι ψευδής από την αρχή, το σώμα του βρόχου δεν εκτελείται **καμία** φορά:
...
Το διάγραμμα ροής της `while`:

```mermaid
graph LR
  A(["είσοδος"]) --> C{"ισχύει η E2"}
  C -- ναι --> B["σώμα του βρόχου"]
  B --> C
  C -- όχι --> Z(["έξοδος"])
```

Η `do...while` ελέγχει τη συνθήκη **μετά** από κάθε επανάληψη. Το σώμα της εκτελείται επομένως **τουλάχιστον μία** φορά, ακόμα κι αν η `E2` είναι εξαρχής ψευδής:
...
Στο διάγραμμα ροής της φαίνεται καθαρά η διαφορά από τη `while`: το σώμα του βρόχου βρίσκεται **πριν** από τον έλεγχο και όχι μετά.

```mermaid
graph LR
  A(["είσοδος"]) --> B["σώμα του βρόχου"]
  B --> C{"ισχύει η E2"}
  C -- ναι --> B
  C -- όχι --> Z(["έξοδος"])
```

Η `for`, τέλος, μαζεύει την αρχικοποίηση, τον έλεγχο και το βήμα σε μία γραμμή:
...
- `E1` - **εντολή αρχικοποίησης**: εκτελείται μία μόνο φορά, πριν από την πρώτη επανάληψη.
- `E2` - **συνθήκη εισόδου και συνέχισης**: όσο είναι αληθής, ο βρόχος επαναλαμβάνεται.
- `E3` - **εντολή επανάληψης**: εκτελείται στο τέλος κάθε επανάληψης, πριν από τον επόμενο έλεγχο της `E2`.

Το διάγραμμα ροής της `for` δείχνει πότε εκτελείται η καθεμιά: η `E1` μία μόνο φορά στην αρχή, η `E2` πριν από κάθε επανάληψη και η `E3` στο τέλος κάθε επανάληψης, αμέσως πριν από τον επόμενο έλεγχο:
...
Η `for` δεν προσθέτει κάποια δυνατότητα που δεν έχει ήδη η `while` - είναι απλώς πιο συμπαγής γραφή. Ο παραπάνω βρόχος `for` είναι ισοδύναμος με τον:
...
- [Άσκηση 1: Υπολογισμός Αθροίσματος Σειράς (seq.c)](#άσκηση-1-υπολογισμός-αθροίσματος-σειράς-seqc)
  - [1.1. Γράψτε μια συνάρτηση `sum` που να υπολογίζει το άθροισμα της σειράς:](#11-γράψτε-μια-συνάρτηση-sum-που-να-υπολογίζει-το-άθροισμα-της-σειράς)
  - [1.2. Γράψτε μια συνάρτηση `basel` που να υπολογίζει το άθροισμα της σειράς:](#12-γράψτε-μια-συνάρτηση-basel-που-να-υπολογίζει-το-άθροισμα-της-σειράς)
  - [1.3. Γράψτε μια συνάρτηση `pi_approx` που να προσεγγίζει το $\pi$ χρησιμοποιώντας την έκφραση:](#13-γράψτε-μια-συνάρτηση-pi_approx-που-να-προσεγγίζει-το-pi-χρησιμοποιώντας-την-έκφραση)
  - [1.4. Υλοποιήστε μια συνάρτηση `eta_two` η οποία να υπολογίζει το άθροισμα της σειράς:](#14-υλοποιήστε-μια-συνάρτηση-eta_two-η-οποία-να-υπολογίζει-το-άθροισμα-της-σειράς)
  - [1.5. Εξάσκηση σε Σειρές](#15-εξάσκηση-σε-σειρές)
- [Άσκηση 2: Υπολογισμός ριζών τριωνύμου (root.c)](#άσκηση-2-υπολογισμός-ριζών-τριωνύμου-rootc)
  - [2.1. Παραγωγή τυχαίων αριθμών](#21-παραγωγή-τυχαίων-αριθμών)
  - [2.2. Εύρεση ριζών](#22-εύρεση-ριζών)
  - [2.3. Μιγαδικές Ρίζες](#23-μιγαδικές-ρίζες)
- [Άσκηση 3: Υπολογισμός ημέρας μίας δεδομένης ημερομηνίας (birthdate.c)](#άσκηση-3-υπολογισμός-ημέρας-μίας-δεδομένης-ημερομηνίας-birthdatec)
- [ΠΑΡΑΡΤΗΜΑ: Αποσφαλμάτωση προγραμμάτων (Πράξη 2η)](#παραρτημα-αποσφαλμάτωση-προγραμμάτων-πράξη-2η)
  - [Λογικά λάθη](#λογικά-λάθη)
  - [Τεχνικές εύρεσης και διόρθωσης λογικών λαθών](#τεχνικές-εύρεσης-και-διόρθωσης-λογικών-λαθών)
  - [Άσκηση Αποσφαλμάτωσης (limit.c)](#άσκηση-αποσφαλμάτωσης-limitc)
...
- Συνάρτηση `harmonic` [[3]](https://en.wikipedia.org/wiki/Harmonic_series_(mathematics)#Alternating_harmonic_series) που υπολογίζει το άθροισμα της εναλλάσσουσας αρμονικής σειράς:
...
Όπου, με $\lfloor x \rfloor$ συμβολίζουμε τον μέγιστο ακέραιο αριθμό που δεν είναι μεγαλύτερος του αριθμού $x$ - γνωστή και ως συνάρτηση `floor` ([αντίστοιχα υπάρχει και η συνάρτηση ceil](https://en.wikipedia.org/wiki/Floor_and_ceiling_functions)). Δεν είμαστε σίγουροι πώς λειτουργεί - πώς θα το βρούμε;
...
Ο παραπάνω υπολογισμός φαίνεται περίπλοκος; Δεν έχετε δει τίποτε ακόμη! Πραγματικές συναρτήσεις βιβλιοθήκης ημερομηνιών πρέπει να χειρίζονται ημερομηνίες και στο παρελθόν, με ιδιαίτερα περίπλοκες αλλαγές - τι γίνεται για παράδειγμα αν ο χρήστης θέλει να βρει την ημερομηνία 16 Φεβρουαρίου 1923 στην Ελλάδα [[8]](https://en.wikipedia.org/wiki/Adoption_of_the_Gregorian_calendar);
...
> "Σε μια μεταβλητή `c` θα κρατάω τον τρέχοντα όρο που θα προστεθεί, ο οποίος παράγεται από τον προηγούμενο αν πολλαπλασιάσω με ½. Θέλω ανά πάσα στιγμή ο όρος `c` να μην ξεπερνά το `LIMIT`. Το `c` έχει διαφορετικό πρόσημο σε κάθε όρο, οπότε θα χρησιμοποιήσω μια μεταβλητή `sign` που θα την πολλαπλασιάζω με το -1 ώστε ανά δύο όρους να έχει το ίδιο πρόσημο. Τελικά θα εκτυπώσω το άθροισμα `S`".
