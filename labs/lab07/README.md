...
>
> - Να ορίζετε και να διασχίζετε πολυδιάστατους πίνακες.
> - Να δεσμεύετε και να αποδεσμεύετε μνήμη δυναμικά με `malloc` και `free`.
> - Να δεσμεύετε δυναμικά δισδιάστατους πίνακες όταν οι διαστάσεις δεν είναι γνωστές εκ των προτέρων.
> - Να εντοπίζετε σφάλματα διαχείρισης μνήμης με τη βοήθεια του `gdb`.
>
> **Προαπαιτούμενα:** [Εργαστήριο #6](https://progintro.github.io/lab-material/labs/lab06/) - δείκτες και πίνακες.
>
> **Αρχεία που θα φτιάξετε:** `twodim.c`, `array.c`, `mines.c`, `olaf.c`, `pacman.c`, `my_prog.c`
...
- [Άσκηση 1: Δισδιάστατοι πίνακες (twodim.c)](#άσκηση-1-δισδιάστατοι-πίνακες-twodimc)
- [Άσκηση 2: Δυναμική δέσμευση μνήμης για μονοδιάστατο πίνακα (array.c)](#άσκηση-2-δυναμική-δέσμευση-μνήμης-για-μονοδιάστατο-πίνακα-arrayc)
- [Άσκηση 3: Δυναμική δέσμευση μνήμης για δισδιάστατο πίνακα (mines.c)](#άσκηση-3-δυναμική-δέσμευση-μνήμης-για-δισδιάστατο-πίνακα-minesc)
  - [Δυναμική δέσμευση μνήμης για δισδιάστατο πίνακα διάστασης N x M](#δυναμική-δέσμευση-μνήμης-για-δισδιάστατο-πίνακα-διάστασης-n-x-m)
  - [Αποδέσμευση μνήμης για δυναμικά δεσμευμένο πίνακα N x M](#αποδέσμευση-μνήμης-για-δυναμικά-δεσμευμένο-πίνακα-n-x-m)
- [Άσκηση 4 (Παλιό θέμα): Χτίζοντας έναν χιονάνθρωπο (olaf.c)](#άσκηση-4-παλιό-θέμα-χτίζοντας-έναν-χιονάνθρωπο-olafc)
- [Άσκηση 5 (Παλιό θέμα): Κινήσεις σε πλέγμα (pacman.c)](#άσκηση-5-παλιό-θέμα-κινήσεις-σε-πλέγμα-pacmanc)
- [Παράρτημα: Αποσφαλμάτωση προγραμμάτων (Πράξη 3η)](#παράρτημα-αποσφαλμάτωση-προγραμμάτων-πράξη-3η)
  - [Σφάλματα διαχείρισης μνήμης](#σφάλματα-διαχείρισης-μνήμης)
  - [Ο debugger gdb](#ο-debugger-gdb)
  - [Άσκηση 6: Εντοπισμός σφάλματος μνήμης με τον gdb (my_prog.c)](#άσκηση-6-εντοπισμός-σφάλματος-μνήμης-με-τον-gdb-my_progc)

<!-- /toc -->

## Άσκηση 1: Δισδιάστατοι πίνακες (twodim.c)
...
**1.4** Επεκτείνετε το πρόγραμμα `twodim.c` ώστε να εκτυπώνει όλα τα στοιχεία του πίνακα σε μία σειρά, διασχίζοντάς τον με έναν [οφιοειδή](https://el.wiktionary.org/wiki/%CE%BF%CF%86%CE%B9%CE%BF%CE%B5%CE%B9%CE%B4%CE%AE%CF%82) τρόπο, όπως φαίνεται στο παράδειγμα:
...
>
> - `ΤΔ *p;` — δήλωση ενός δείκτη σε στοιχεία τύπου *ΤΔ*
> - `p = malloc(N * sizeof(ΤΔ));` — δέσμευση μνήμης για N στοιχεία τύπου *ΤΔ* Η `malloc` επιστρέφει `NULL` σε περίπτωση αποτυχίας δέσμευσης της αιτούμενης μνήμης.
...
## Άσκηση 3: Δυναμική δέσμευση μνήμης για δισδιάστατο πίνακα (mines.c)

**3.1** Κατασκευάστε το πρόγραμμα `mines.c` που να διαβάζει από την είσοδο τις διαστάσεις ενός δισδιάστατου πίνακα χαρακτήρων (έστω `N x M`), να δεσμεύει χώρο `N x M` θέσεων δυναμικά και έπειτα να τον αρχικοποιεί διαβάζοντας από την είσοδο χαρακτήρες.
...
## Άσκηση 4 (Παλιό θέμα): Χτίζοντας έναν χιονάνθρωπο (olaf.c)
...
## Άσκηση 5 (Παλιό θέμα): Κινήσεις σε πλέγμα (pacman.c)
...
Ενώ είναι εφικτό η ανίχνευση των σφαλμάτων διαχείρισης μνήμης να γίνει με εξαντλητική ιχνηλάτηση του κώδικα και χρήση `printf`, ο τρόπος αυτός είναι πολύ αναποτελεσματικός και κουραστικός. Για αυτό το λόγο, αλλά και για την ευκολότερη ανίχνευση και των λογικών λαθών στα οποία αναφερθήκαμε σε προηγούμενα εργαστήρια, θα δούμε στη συνέχεια πώς δουλεύει ένα εργαλείο αποσφαλμάτωσης (debugging), ο `gdb`. Το εργαλείο αυτό θα κάνει την ανίχνευση και τη διόρθωση των λογικών λαθών και των σφαλμάτων διαχείρισης μνήμης πολύ πιο εύκολη, ενώ δεν θα απαιτεί να πειράξουμε τον κώδικα για την εισαγωγή `printf`.
...
Όταν η εκτέλεση του προγράμματος έχει ανασταλεί, μπορούμε να συνεχίσουμε την εκτέλεση βηματικά, δηλαδή να εκτελείται μόνο μία γραμμή κώδικα κάθε φορά. Με την εντολή `s` εκτελείται η τρέχουσα γραμμή κώδικα, ενώ αν αυτή είναι η κλήση κάποιας συνάρτησης, ο έλεγχος μεταφέρεται εντός της.
...
Τρέχοντας αυτό το πρόγραμμα, αργά ή γρήγορα θα μας δώσει segmentation fault. Ας τρέξουμε λοιπόν τον `gdb` για να μας βοηθήσει.
...
To `$3` δεν πρέπει να σας απασχολεί. Αυτό που μας ενδιαφέρει είναι η αριθμητική τιμή που εμφανίζεται, το `1279`. Άρα το `i` έχει φτάσει στο `1279`! Είναι προφανές ότι κάπου μέσα στο πρόγραμμά μας έχουμε ξεχάσει να κάνουμε έλεγχο για να μην ξεπερνάμε τα όρια του πίνακα p. Όντως, πουθενά στον κώδικά μας δεν ελέγχουμε αν είμαστε μέσα στα όρια του πίνακα. Δεδομένου ότι το μέγεθος του πίνακα p δεν δηλώνεται ρητά, πώς θα αντιμετωπίζατε αυτό το πρόβλημα; Ήταν αυτό αρκετό για να λειτουργήσει σωστά το πρόγραμμα; Αν φταίει και κάτι άλλο, προσπαθήστε να το βρείτε με τη χρήση του gdb.

### Άσκηση 6: Εντοπισμός σφάλματος μνήμης με τον gdb (my_prog.c)
