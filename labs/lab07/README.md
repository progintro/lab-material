...
ώστε να εκτυπώνει τα στοιχεία κάθε γραμμής του αρχικού πίνακα με αντίστροφη
σειρά. Για παράδειγμα, αν ο αρχικός σας πίνακας είναι και πάλι:
...
στοιχεία του πίνακα σε μία σειρά, διασχίζοντάς τον με έναν [οφιοειδή](https://el.wiktionary.org/wiki/%CE%BF%CF%86%CE%B9%CE%BF%CE%B5%CE%B9%CE%B4%CE%AE%CF%82)
...
στοιχείων (`N`) και ακολουθούν οι `N` ακέραιοι χωρισμένοι με κενά.
...
τις διαστάσεις ενός δισδιάστατου πίνακα χαρακτήρων (έστω `N x M`), να
...
κλείσιμο του προγράμματος αν το τρέχουμε μέσα από κάποιο IDE). Γενικά,
...
τουλάχιστον ενός breakpoint είναι απαραίτητη, γιατί αλλιώς δεν θα
...
είναι η κλήση κάποιας συνάρτησης, ο έλεγχος μεταφέρεται εντός της.
...
Τρέχοντας αυτό το πρόγραμμα, αργά ή γρήγορα θα μας δώσει segmentation
...
Δεδομένου ότι το μέγεθος του πίνακα p δεν δηλώνεται ρητά, πώς θα
αντιμετωπίζατε αυτό το πρόβλημα; Ήταν αυτό αρκετό για να λειτουργήσει
