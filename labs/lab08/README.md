...
> Προσοχή: τον χαρακτήρα `*` τον επεκτείνει το shell πριν φτάσει στο
> πρόγραμμά μας, οπότε για τον πολλαπλασιασμό τον γράφουμε σε εισαγωγικά
> (`./calc 6 '*' 7`) ή με ανάποδη κάθετο (`./calc 6 \* 7`).
...
- Η μεταβλητή `argc` αρχικοποιείται με το πλήθος των ορισμάτων `N + 1`,
...
- Η συνάρτηση `int atoi(const char *s)` επιστρέφει στο όνομά της την
...
Στο προηγούμενο εργαστήριο είδαμε ένα εργαλείο για την αποσφαλμάτωση προγραμμάτων,
...
πρόβλημά μας είναι να αλλάξουμε αυτή τη γραμμή κώδικα:
